                ...
                <div class="panel" style="display: flex; flex-direction: column; gap: 16px; min-width: 0; background: rgba(0,0,0,0.1); border-right: 1px solid var(--border);">
                    ...
                    <div style="flex: 1; display: flex; flex-direction: column; gap: 8px; overflow-y: auto; max-height: 480px; padding-right: 4px;">
                        <div v-for="item in filteredQuickInsertItems" :key="item.id || item.name"
                            ...
                            <img v-if="item._quickType === 'product' || item._quickType === 'service'"
                                 :src="app.getCleanImageUrl(getProductImageUrl(item))"
                    ...
                    <div class="form-group" style="position: relative;">
                        <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 6px; flex-wrap: wrap; gap: 8px;">
                            ...
                            <span style="font-size: 0.72rem; color: var(--text-muted); font-weight: 500;">
                                💡 Tip: Type <strong style="color: var(--accent);">@</strong> for personas/services, <strong style="color: var(--accent);">#</strong> for sceneries, or <strong style="color: var(--accent);">$</strong> for products.
                            </span>
                        </div>
                        ...
                        <div style="position: relative; width: 100%; margin-bottom: 12px;">
                            ...
                            <div v-if="autocomplete.active" 
                                ...
                                <div style="font-size: 0.7rem; color: var(--accent); font-weight: 700; margin-bottom: 4px; text-transform: uppercase; display: flex; justify-content: space-between; align-items: center;">
                                    ...
                                    <span v-if="replacingTagElement" @click="removeReplacingTag" style="cursor: pointer; color: #ef4444; font-size: 0.65rem; text-decoration: underline;">Remove Tag</span>
                                </div>
                                ...
                                <div style="display: flex; flex-direction: column; gap: 4px; overflow-y: auto; max-height: 140px;">
                                    <div v-for="(item, idx) in filteredAutocompleteItems" :key="item.id || item.name" 
                                         @click="insertAutocompleteTag(item)"
                                         :style="{
...
export default {
    ...
    data() {
        return {
            ...
            // Loaded interactive canvas elements
            canvas: {
                brand_voice: '',
                archetype: 'creator',
                personas: [],
    ...
    computed: {
        ...
        filteredAutocompleteItems() {
            ...
            if (this.autocomplete.type === 'product') {
                ...
                return rank(prods, 'product').slice(0, 10);
            } else if (this.autocomplete.type === 'service') {
                if (!this.app.products) return [];
                const svcs = this.app.products.filter(p => p && p.type === 'service');
                return rank(svcs, 'service').slice(0, 10);
            } else if (this.autocomplete.type === 'persona') {
                const pers = (this.canvas.personas || []).map(p => ({ ...p, _quickType: 'persona' }));
                const svcs = (this.app.products || []).filter(p => p && p.type === 'service').map(s => ({ ...s, _quickType: 'service' }));
                const auds = (this.audiences || []).map(a => ({ ...a, _quickType: 'audience' }));
                const combined = [...pers, ...svcs, ...auds];
                return rank(combined, 'persona').slice(0, 15);
    ...
    methods: {
        ...
        handleEditorClick(e) {
            ...
            if (tagEl) {
                ...
                this.autocomplete.activeSymbol = this.autocomplete.type === 'persona' ? '@' : (this.autocomplete.type === 'product' ? '$' : '#');
        ...
        updateAutocompleteState() {
            ...
            if (triggerIndex !== -1) {
                ...
                this.autocomplete.active = true;
                this.autocomplete.type = activeSymbol === '@' ? 'persona' : (activeSymbol === '$' ? 'product' : (activeSymbol === '%' ? 'service' : 'scenery'));
        ...
        insertRichTag(type, item) {
            ...
            // Update savedRange after insertion
            const selection = window.getSelection();
        ...
        randomizeComposerScene() {
            ...
            const personasList = this.canvas.personas || [];
            const sceneriesList = this.canvas.sceneries || [];
        ...
        deleteSeedPreset(preset) {
            this.presetAnchors = this.presetAnchors.filter(p => p.seed !== preset.seed);
            this.app.showNotification(`🗑️ Anchor preset removed.`);
        },
        saveCurrentSeedAsPreset() {
            ...
            this.presetAnchors.push({ name, seed });
        ...
        async generateGuidelineAssetAi(type) {
            ...
            const brandId = this.settingsBrand.id;
            if (!brandId) return;
        ...
        async loadBrandCanvas() {
            ...
            try {
                ...
                if (audResp.ok) {
                    ...
                    this.audiences = audData.audiences || [];
                }
            } catch (err) {
        ...
        async saveQuickCreateItem() {
            ...
            try {
                if (form.type === 'product' || form.type === 'service') {
                    ...
                    if (response.ok) {
                        this.app.showNotification('Successfully added audience segment');
